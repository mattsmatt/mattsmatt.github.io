...
function SchoolItem({ school }) {
  ...
  return (
    <>
      <div
        className={`flex flex-col gap-5 md:items-start w-full md:justify-between py-8 px-4 md:flex-row`}
      >
        <div className="w-1/2 m-auto md:w-[10%] md:m-0">
          <img
            src={getImageUrl(school.image, "school")}
            alt=""
            className="w-full"
          />
        </div>
        <div className="w-full md:w-[85%]">
          ...
          <div className="opacity-75">
            <div>{school.degree + " degree, " + school.field}</div>
          </div>
          <div>
            <div>
              {new Date(school.dateStart).toLocaleString("en-US", options) +
                " - " +
                new Date(school.dateEnd).toLocaleString("en-US", options)}
            </div>
          </div>
          <div>
          ...
          <div
            className={school.importantLinks.length == 0 ? "hidden" : "block"}
          >
            <ul className="flex gap-2 my-2">
              {school.importantLinks.map((link) => (
